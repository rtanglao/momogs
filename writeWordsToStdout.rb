...
while true
  ...
  topics["data"].each do|topic|
    ...
    topic_text = Sanitize.clean(topic["subject"].downcase + " " + topic["content"].downcase)
    ...
    if reply_count != 0
      begin # while reply_count > 0
        ...
        replies["data"].each do|reply|
          ...
          topic["fulltext"] = topic["fulltext"] + " " +  Sanitize.clean(reply["content"].downcase)
        ...
        begin  # while tag_count > 0         
          ...
          tags["data"].each do|tag|    
            ...
            if tag_name.length < 80
              topic["fulltext_with_tags"] = topic["fulltext_with_tags"] + " " + tag_name.gsub(/ /, '~') # for wordle, changes spaces to tildes
            end
          end # tags ... do
          tag_count -= 30
          tags_page += 1                   
